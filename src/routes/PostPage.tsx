...
export default function PostPage() {
  ...
  if (item) {
    return (
      <>
        <div className="flex items-start justify-between gap-4" data-aos="fade-in">
          <h3 className="text-xl font-semibold pl-6">{item.title}</h3>
          <button
            type="button"
        ...
        <div className="mt-3 grid grid-cols-1 gap-4 md:grid-cols-2" data-aos="fade-in">
          <img
            src={item.screenshot}
            alt={item.title}
          ...
          />
          <div>
            <h4 className="text-lg font-semibold pb-1 pt-2 ">Case study</h4>
            <p className="text-sm text-neutral-600 dark:text-neutral-300">{item.caseStudy}</p>
            <h4 className="text-lg font-semibold pt-2 pb-1">Outcomes</h4>
            <ul className="list-disc">
              {Object.values(item.outcomes || {}).map((outcome, index) => (
                <li key={index} className="text-sm text-neutral-600 dark:text-neutral-300">{outcome}</li>
              ))}
            </ul>
            <h4 className="text-lg font-semibold pt-2 pb-1">Stack</h4>
